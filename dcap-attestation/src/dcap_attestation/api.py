...
class VerificationResponse(BaseModel):
    ...
    quote_collateral: Optional[QuoteCollateralV3] = None


async def get_quote_collateral(quote: Quote):
...
async def verify(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    if record.success:
        ...
        if quote.verified:
            record.quote_collateral = await get_quote_collateral(quote)
...
async def get_collateral(checksum: str, db: Session = Depends(get_db)):
    ...
    collateral = await get_quote_collateral(quote)
    return JSONResponse(
